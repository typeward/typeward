...
- **Deliberately deferred** (honestly badged "soon" in the UI; build only on explicit request): notifications system, pandoc docx/html exports, PDF annotation flattening, Stripe webhook (7.6), tablet layouts for Projects/Settings screens.
...
- **Stripe webhook edge function** (Integ Phase 7.6) — lands once Stripe products + prices exist. Schema already carries `stripe_subscription_id` / `stripe_customer_id` for the webhook to upsert via service-role; until then `subscriptions` rows are inserted by hand in Supabase Studio or via `seed_test_users.sql`.

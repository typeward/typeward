...
// element of each `then_kind_any` / `then_kind_any_but_not` predicate array
// carries an added `as fn(&TokenKind) -> bool` cast (two sites) so rustc >=
// ~1.93 coerces the distinct fn-item types into one fn-pointer type (E0308).

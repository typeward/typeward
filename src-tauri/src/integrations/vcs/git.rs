...
mod tests {
    ...
    fn remote_url_validation_accepts_https() {
        assert!(validate_remote_url("https://github.com/typeward/typeward.git").is_ok());
    }

    #[test]
    fn remote_url_validation_rejects_file_and_ssh() {
        assert!(validate_remote_url("file:///tmp/repo.git").is_err());
        assert!(validate_remote_url("git@github.com:typeward/typeward.git").is_err());
        assert!(validate_remote_url("ssh://git@github.com/typeward/typeward.git").is_err());
    }
}

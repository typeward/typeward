...
pub fn run() {
    ...
    let builder = builder
        .setup(|app| {
            ...
            // Mint this install's identity before the webview can issue IPC, so
            // every review comment written this session lands under a settled
            // author id and the reviews sidecar never has to guess.
            if let Err(e) = settings::ensure_local_identity(app.handle()) {
                eprintln!("[typeward] could not establish the local profile id: {e}");
            }
    ...
    let commands: fn(tauri::ipc::Invoke<tauri::Wry>) -> bool = tauri::generate_handler![
        ...
        install_kind::updater_install_kind,
        ...
        integrations::webdav::webdav_mkcol,

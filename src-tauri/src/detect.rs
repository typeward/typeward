...
pub struct EngineProbe {
    ...
    /// True when the Tectonic sidecar file ships next to this executable.
    pub tectonic_bundled: bool,
}
...
pub fn probe() -> EngineProbe {
    ...
    EngineProbe {
        engines,
        any_latex_available,
        // Whether THIS build actually ships the sidecar, not whether tectonic
        // is reachable: Windows/Linux ARM64 builds exclude it on purpose (the
        // tauri.no-tectonic.conf.json overlay), and a dev tree before
        // `npm run fetch:tectonic` doesn't have it either. Onboarding copy
        // must not promise a bundled engine that isn't there.
        tectonic_bundled: bundled_sidecar("tectonic").is_some(),
    }
}

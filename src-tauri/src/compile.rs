...
async fn run_tectonic(
    ...
    let tectonic = match crate::detect::resolve_program("tectonic") {
        Ok(path) => path,
        Err(_) => {
            // Windows/Linux ARM64 builds ship without the sidecar on purpose,
            // so this is a normal end-user path there, not only a dev tree
            // before `npm run fetch:tectonic`. Say what to do, not what CI
            // didn't run.
            return Err(
                "Tectonic is not included in this build and was not found on PATH. \
                 Install tectonic, or pick a different engine in Settings. \
                 (Developers: run `npm run fetch:tectonic`.)"
                    .into(),
            );
        }
    };

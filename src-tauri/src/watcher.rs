...
pub struct WatcherEvent {
    ...
    /// The project's review shards changed on disk. Carried as a flag rather
    /// than as a path because the sidecar is filtered out of `paths` on
    /// purpose (it must never wake the FileTree), yet a shard write is the one
    /// piece of sidecar churn the app has to react to: in a folder-synced
    /// project it is a collaborator's comment arriving.
...
fn watch_project_impl(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if !paths.is_empty() || reviews_changed {
...
/// True for a write inside `<project>/.typeward/reviews/`. Still "internal" as
/// far as [`is_internal_path`] and the FileTree are concerned; this only
/// distinguishes the one sidecar the review store has to re-read.
fn is_review_shard_path(path: &str) -> bool {
    path.replace('\\', "/").contains("/.typeward/reviews/")
}
...
mod tests {
    ...
    #[test]
    fn review_shard_writes_are_internal_but_still_recognized() {
        // Both must hold: the FileTree never sees the sidecar, and the review
        // store still learns that a collaborator's shard landed.
        for p in [
            "/home/u/proj/.typeward/reviews/V1StGXR8Z5jdHi6BmyT8x.json",
            "C:\\Users\\u\\proj\\.typeward\\reviews\\aBcDeF.json",
        ] {
            assert!(is_internal_path(p), "{p} must stay out of the tree");
            assert!(is_review_shard_path(p), "{p} must wake the review store");
        }
        // Other sidecar churn must NOT wake it, or autosave becomes a reload loop.
        for p in [
            "/home/u/proj/.typeward/snapshots/main.tex.snap",
            "/home/u/proj/.typeward/build/main.pdf",
            "/home/u/proj/reviews/notes.tex",
        ] {
            assert!(
                !is_review_shard_path(p),
                "{p} must not wake the review store"
            );
        }
    }

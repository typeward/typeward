...
/// seeds a template's `author` variable and attributes review comments.
/// Nothing here is transmitted. (Git commit identity comes from the user's own
/// gitconfig, never from here.)
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ProfileSettings {
    ...
    /// Stable per-install identifier, minted once by [`ensure_local_identity`].
    /// It distinguishes *this* install's review comments from a collaborator's
    /// in a folder-synced project, and names this install's review shard file.
    /// Backend-owned, like `avatar_path`: the renderer round-trips it but never
    /// mints or clears it, so a payload that omits it must not wipe it (see
    /// [`merge_backend_owned`]). Never leaves the device.
...
fn merge_backend_owned(incoming: &mut Settings, existing: &Settings) {
    ...
    if incoming.profile.local_id.is_none() {
        incoming.profile.local_id = existing.profile.local_id.clone();
    }
}

/// nanoid's default alphabet, so an id minted here is indistinguishable from
/// one the frontend mints and both sides can validate with the same pattern.
const ID_ALPHABET: &[u8; 64] = b"_-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
const ID_LEN: usize = 21;

fn mint_local_id() -> String {
    use rand::Rng;
    let mut bytes = [0u8; ID_LEN];
    rand::rng().fill_bytes(&mut bytes);
    // 256 is a multiple of 64, so masking is a uniform draw, not a modulo bias.
    bytes
        .iter()
...
/// True for an id shaped like [`mint_local_id`]'s output. Review shard files are
/// named `<local_id>.json`, and those names come off the filesystem (a
/// collaborator's shard arrives through folder sync), so every consumer
/// re-validates rather than trusting the name.
...
/// The OS account name, used only to seed an empty display name on first run.
/// Falls back to an empty string, which every consumer already handles.
...
/// Mint the per-install identity if it is missing, and seed an empty display
/// name from the OS account. Runs once at startup, before the webview can issue
/// IPC, so every later reader sees a settled `local_id`.
///
/// Both halves are first-run seeding, never correction: a user who clears their
/// display name keeps it cleared, because only an *absent* `local_id` triggers
/// the write at all.
pub fn ensure_local_identity(app_handle: &tauri::AppHandle) -> Result<(), SettingsError> {
    let _guard = lock_settings();
    let settings = load_locked(app_handle)?;
    if settings
        .profile
        .local_id
        .as_deref()
        .is_some_and(is_valid_local_id)
    {
        return Ok(());
    }
    let mut settings = settings;
    settings.profile.local_id = Some(mint_local_id());
    if settings.profile.display_name.trim().is_empty() {
        settings.profile.display_name = os_account_name();
    }
    save_locked(app_handle, &settings)
}
...
mod tests {
    ...
    #[test]
    fn the_stored_local_id_survives_a_frontend_settings_roundtrip() {
        let mut existing = Settings::default();
        existing.profile.local_id = Some("V1StGXR8Z5jdHi6BmyT8x".into());

        // The renderer echoes the profile back without minting an id of its own;
        // dropping it here would re-key every review comment this install owns.
        let mut payload = serde_json::to_value(Settings::default()).unwrap();
        payload["profile"]["displayName"] = serde_json::json!("Ada");
        let mut incoming: Settings = serde_json::from_value(payload).unwrap();
        assert_eq!(incoming.profile.local_id, None);

        merge_backend_owned(&mut incoming, &existing);
        assert_eq!(
            incoming.profile.local_id.as_deref(),
            Some("V1StGXR8Z5jdHi6BmyT8x")
        );
    }

    #[test]
    fn a_minted_local_id_is_shaped_like_nanoid_and_validates() {
        let a = mint_local_id();
        let b = mint_local_id();
        assert_eq!(a.len(), ID_LEN);
        assert_ne!(a, b);
        assert!(is_valid_local_id(&a));
    }

    #[test]
    fn local_id_validation_rejects_anything_that_is_not_a_bare_file_stem() {
        // Shard file names come off the filesystem, so a traversal or extension
        // smuggled into one must never be handed back as a readable path.
        for bad in [
            "",
            "../secret",
            "a/b",
            ...
            assert!(!is_valid_local_id(bad), "{bad} should be rejected");
        }
        for ok in ["V1StGXR8Z5jdHi6BmyT8x", "a", "A-Z_0-9"] {
            assert!(is_valid_local_id(ok), "{ok} should pass");
        }
    }

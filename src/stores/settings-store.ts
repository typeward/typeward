...
/** The user-editable half of the profile. `avatarPath` and `localId` are
 *  deliberately absent: Rust owns both (see `profileAvatarPath`,
 *  `profileLocalId`). */
export type ProfileFields = Omit<ipc.ProfileSettings, "avatarPath" | "localId">;
...
// Read-only mirror of profile.localId, minted once by Rust at startup. It
// attributes this install's review comments and names its review shard, so it
// must survive a settings roundtrip untouched — hence the same
// backend-owned treatment as the avatar path, not a ProfileFields member the
// renderer could overwrite. Empty until settings have loaded.
const [profileLocalId, setProfileLocalId] = createSignal<string>("");
...
const FIELDS: FieldSpec[] = [
  ...
  // --- profile ---
  // Not the generic field() shape: the avatar path and the local id hydrate
  // into their own signals and are deliberately left out of the serialized
  // object, so Rust restores the values it owns instead of taking
  // renderer-invented ones.
  {
    key: "profile",
    hydrate: (s) => {
      setProfile({
      ...
      setProfileLocalId(s.profile?.localId ?? "");
    },
    serialize: (out) => {
      out.profile = { ...profile() };
    },

...
describe("createWebdavProvider listRoots", () => {
  it("lists only the directories inside the shared Typeward folder", async () => {
    propfindFrom({
      Typeward: [
        dir("Typeward/Paper"),
        dir("Typeward/Notes"),
        file("Typeward/readme.md", { etag: "e" }),
      ],
      "": [dir("Unrelated")],
    });
    const roots = await createWebdavProvider(ACCOUNT).listRoots();
    expect(roots).toEqual([
      { id: "Typeward/Paper", name: "Paper" },
      { id: "Typeward/Notes", name: "Notes" },
    ]);
  });

  it("reads a missing Typeward folder as empty, not an error", async () => {
    vi.mocked(webdavPropfind).mockRejectedValueOnce(new Error("WebDAV 404 Not Found"));
    await expect(createWebdavProvider(ACCOUNT).listRoots()).resolves.toEqual([]);
  });

  it("still surfaces a non-404 listing failure", async () => {
    vi.mocked(webdavPropfind).mockRejectedValueOnce(new Error("WebDAV 401 Unauthorized"));
    await expect(createWebdavProvider(ACCOUNT).listRoots()).rejects.toThrow("401");
  });
});

describe("createWebdavProvider ensureFolder", () => {
  it("MKCOLs the folder without its surrounding slashes", async () => {
    vi.mocked(webdavMkcol).mockResolvedValueOnce(undefined);
    await createWebdavProvider(ACCOUNT).ensureFolder("/Typeward/My Thesis/");
    expect(webdavMkcol).toHaveBeenCalledWith(ACCOUNT, "Typeward/My Thesis");
  });
});

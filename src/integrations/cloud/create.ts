 ...
 *
 * The remote root is `Typeward/<project name>` (see `core/remote-root.ts`).
 *
 ...
 *   2. creates the remote folder if it isn't there yet
 *   3. SEEDS the local cache from existing remote content first — downloads
 ...
 *   4. writes project metadata: detected from the seeded content when the
 *      remote had files, or a fresh starter shell when the remote was empty
 *   5. records the `cloudOrigin` binding
 *
...
export async function createCloudBackedProject(
  ...
  // 1. Plant the remote folder. A new project's folder under `Typeward/` does
  //    not exist yet, and the enumeration below would 404 on it; the call is
  //    idempotent, so re-opening an existing remote folder costs one MKCOL.
  await provider.ensureFolder(opts.remoteRoot.id);

  // 2. Seed the cache from existing remote content before any metadata is
  ...
  // 4. Record the cloud binding so subsequent opens start an engine.

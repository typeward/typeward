...
export class SyncEngine {
  ...
  /** Whether `adoptLegacyStateDir` has already run for this engine. */
  private stateDirAdopted = false;
  ...
  private recordConflictsWithNotice(conflicts: string[]): void {
    ...
    pushNotification({
      ...
      key: `sync-conflict:${this.opts.providerId}:${this.opts.projectId}`,
    });
    notifyError(
  ...
  /**
   * Adopt a state directory named before provider ids were escaped for the
   * filesystem (`providerStateSegment`). The raw id is a legal directory name
   * on macOS and Linux, so an install that already synced there keeps its
   * cursor and manifest under it, and reading past them would look like a first
   * run — the destructive whole-project re-sync the two loaders below refuse to
   * do. Runs once per engine, ahead of either load.
   *
   * A rename failure propagates: `start()` reports it on the badge and retries
   * on reopen, which beats silently continuing from an empty baseline. The
   * legacy probe cannot throw its way there, though: on Windows the old name is
   * invalid, so `safeExists` swallowing the syntax error is the correct answer.
   */
  private async adoptLegacyStateDir(): Promise<void> {
    if (this.stateDirAdopted) return;
    this.stateDirAdopted = true;
    const legacy = legacyProviderStateDir(this.cacheRoot(), this.opts.providerId);
    const current = providerStateDir(this.cacheRoot(), this.opts.providerId);
    if (legacy === current) return;
    if (!(await safeExists(legacy)) || (await safeExists(current))) return;
    await mkdirParents(current);
    await rename(legacy, current);
  }

  private async ensureCursorLoaded(): Promise<void> {
    if (this.cursor !== undefined) return;
    await this.adoptLegacyStateDir();

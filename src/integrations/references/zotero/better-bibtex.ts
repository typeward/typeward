...
// BBT's own readiness endpoint — answers 200 only when the plugin is loaded.
const BBT_PROBE_URL = `${BASE}/better-bibtex/cayw?probe=probe`;
...
interface Libraries {
  ...
  /** Zotero's HTTP server is up, so the BBT export endpoints are worth trying. */
  serverUp: boolean;
  localApi: boolean;
}
...
/**
 * `true` if Zotero's local HTTP server is listening, with or without Better
 * BibTeX. ANY HTTP answer proves it: only a transport failure (nothing bound to
 * the port) means Zotero is not running. Deliberately not a status check —
 * Zotero's server answers 404 for an endpoint no plugin claims, and rejects
 * methods an endpoint does not declare, so a stricter probe reported a running
 * Zotero as unreachable and no amount of re-checking could clear it.
 */
export async function probeZoteroServer(): Promise<boolean> {
  try {
    await httpRequest({ method: "GET", url: PROBE_URL });
    return true;
  } catch {
    return false;
  }
...
/**
 * `true` if the Better BibTeX plugin itself answers, as opposed to Zotero's
 * bare HTTP server. Used only to tell the user which citation keys they will
 * get; the export paths still try BBT first and fall back on their own, so a
 * wrong answer here costs a hint, never a key.
 */
export async function probeBetterBibTexPlugin(): Promise<boolean> {
  try {
    const res = await httpRequest({ method: "GET", url: BBT_PROBE_URL });
 ...
 *  `tryBbt` only says the Zotero server is up: whether the plugin is actually
 *  installed shows up as the failed request this falls back from. */
async function exportLibrary(lib: LibraryRef, tryBbt: boolean): Promise<string> {
  if (lib.kind === "user") {
    if (tryBbt) {
      try {
        const res = await httpRequest({
...
export function createBetterBibTexProvider(): CitationProvider {
  ...
  const loadLibraries = async (): Promise<Libraries> => {
    ...
    const [serverUp, localApi] = await Promise.all([
      probeZoteroServer(),
      probeZoteroLocalApi(),
    ]);
    if (!serverUp && !localApi) return { libraries: [], serverUp, localApi };
    const libraries = await discoverLibraries(localApi);
    libCache = { fetchedAt: now, libraries, serverUp, localApi };
    return libCache;
  };
  ...
  const refresh = async (): Promise<Cache> => {
    ...
    const { libraries, serverUp, localApi } = await loadLibraries();
    if (!serverUp && !localApi) {
      throw new Error(
    ...
    for (const lib of libraries) {
      let bibtex: string;
      try {
        bibtex = await exportLibrary(lib, serverUp);
        anySuccess = true;
      } catch {
  ...
  const collectionEntries = async (
    ...
    const entries = tag(await exportCollection(lib, collKey, tryBbt), lib);
  ...
  const libraryEntries = async (
    ...
    if (hit && now - hit.fetchedAt < CACHE_TTL_MS) return hit.entries;
    const entries = tag(await exportLibrary(lib, tryBbt), lib);
  ...
  return {
    ...
    async status(): Promise<ProviderStatus> {
      const { serverUp, localApi } = await loadLibraries();
      return serverUp || localApi ? "ready" : "error";
    },
    ...
    async searchLibrary(query: string, library?: string): Promise<Citation[]> {
      ...
      if (library === undefined) {
        ...
        const { libraries, serverUp } = await loadLibraries();
        const node = resolveNode(library, libraries);
        if (!node) return []; // Not one of this provider's nodes.
        if (node.kind === "collection") {
          pool = await collectionEntries(library, node.lib, node.collKey, serverUp);
        } else {
          pool = await libraryEntries(node.lib, serverUp);
        }
      }

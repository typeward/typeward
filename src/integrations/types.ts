...
export interface CloudFsProvider extends IntegrationProvider {
  category: "cloud";
  /**
   * Existing Typeward project folders on the account — the contents of the
   * shared `Typeward/` folder, not the whole account root. Empty when that
   * folder does not exist yet.
   */
  listRoots(): Promise<RemoteFolder[]>;
  /**
   * Create the folder `id` (and any missing ancestor), idempotently. Called
   * before the first enumeration of a newly created project, whose remote
   * folder does not exist until something is written into it.
   */

...
export const PdfViewer: Component<PdfViewerProps> = (props) => {
  ...
  /**
   * The highlight geometry for one thread, in top-origin PDF points, resolved
   * through the same three tiers the page paints (refined word rects, else the
   * SyncTeX hbox, else the coarse band). The annotated-PDF export reads it so
   * the flattened `/Highlight` lands on exactly what the reader sees here,
   * instead of a bare sticky note at the SyncTeX point.
   */
  const highlightGeometry = (
    threadId: string,
  ): { page: number; rects: PtRect[] } | null => {
    const a = (props.annotations ?? []).find((x) => x.threadId === threadId);
    if (!a) return null;
    const refined = refinedRects().get(threadId);
    if (refined && refined.length > 0) return { page: a.page, rects: refined };
    if (a.box) return { page: a.page, rects: [a.box] };
    const size = pageSizes()[a.page - 1];
    if (!size) return null;
    return {
      page: a.page,
      rects: [
        {
          left: 0,
          top: a.y - HIGHLIGHT_BAND_PT + 2,
          width: size.w,
          height: HIGHLIGHT_BAND_PT * 1.15,
        },
      ],
    };
  };
  ...
  return (
    <div ref={rootRef} class="relative flex h-full min-w-0 flex-col overflow-hidden">
      ...
      >
        ...
        <Show when={!props.embedded}>
          <ExportMenu pdfPath={props.path} highlightGeometry={highlightGeometry} />
        </Show>
 ...
 */
const FromLastBuildRibbon: Component = () => {

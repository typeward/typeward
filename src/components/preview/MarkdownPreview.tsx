...
function buildMd(baseDir: string): MarkdownItInstance {
  ...
  md.renderer.rules.image = (tokens, idx, opts, env, self) => {
    ...
    if (srcAttr >= 0) {
      const rewritten = rewriteImageUrl(String(token.attrs![srcAttr]![1]));
      if (rewritten) {
  ...
  md.renderer.rules.link_open = (tokens, idx, opts, env, self) => {
    ...
    if (hrefAttr >= 0) {
      const sanitized = sanitizeLinkUrl(String(token.attrs![hrefAttr]![1]));
      if (sanitized) {

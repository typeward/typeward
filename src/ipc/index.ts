...
export interface EngineProbe {
  ...
  /** Whether this build ships the Tectonic sidecar (Windows/Linux ARM64 builds do not). */
  tectonicBundled: boolean;
}
...
/**
 * Ids of the review-comment shards present in `<project>/.typeward/reviews/`,
 * one per install that has ever commented on the project. Rust validates every
 * name against the id shape before returning it and folds the pre-shard
 * `comments.json` sidecar into this install's shard on the way, so the caller
 * only ever sees well-formed shards.
 */
export const listReviewShards = (projectRoot: string): Promise<string[]> =>
  invoke("list_review_shards", { projectRoot });
...
export interface ProfileSettings {
  ...
  avatarPath?: string;
  /** Stable per-install id, minted once by Rust at startup. It attributes
   *  review comments and names this install's review shard, so a project
   *  shared through a folder-sync client can tell two people apart.
   *  Backend-owned like `avatarPath` — never minted or cleared here. */
  localId?: string;
}
...
/**
 * How this build was installed, in the updater plugin's own vocabulary:
 * `deb` | `rpm` | `appimage` | `nsis` | `msi` | `app` | `unknown`.
 *
 * The value comes from a string the bundler patches into each binary, and it is
 * what the updater uses to pick a manifest entry (`{os}-{arch}-{installer}`
 * before `{os}-{arch}`). The frontend needs it to explain why a package cannot
 * update itself. `unknown` means an unpatched binary: a dev run, or a binary
 * copied out of its bundle.
 */
export const updaterInstallKind = (): Promise<string> =>
  invoke("updater_install_kind");
...
/** One highlight rect in top-origin, page-relative PDF points. */
...
/** One review comment flattened into a PDF annotation (anchor + text + tint). */
export interface AnnotationInput {
  ...
  title: string;
  body: string;
  /** 1-based page, when the viewer already resolved this thread's geometry. */
  page?: number;
  /** Rects to highlight, top-origin pt. Empty falls back to SyncTeX in Rust. */
  quads?: AnnotationQuad[];
  /** Highlight tint as 0-1 RGB, pre-washed for the PDF's Multiply blend. */
...
/**
 * Place review comments into the compiled PDF as `/Highlight` annotations over
 * the text they anchor to, with the comment as the note body. Geometry comes
 * from the viewer when it has it, else from a SyncTeX forward search in Rust.
 * LaTeX-only (SyncTeX is the fallback mapping).
 */

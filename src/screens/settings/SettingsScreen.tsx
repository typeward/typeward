...
const engineStatusFor = (
  ...
  if (engine === "tectonic") {
    ...
    // The probe already checks the sidecar before PATH, so reaching here means
    // this build does not ship it (ARM64 builds do not) and none is installed.
    return {
      text: "tectonic isn't installed and this build doesn't include it. Install it, or switch the engine",
      tone: "err",
    };
  }
  ...
  if (probe.anyLatexAvailable) {
    return {
      text: "pdflatex only: latexmk is missing, so compiling calls pdflatex directly",
      tone: "warn",
    };
  }
  // Only point at Tectonic when this machine can actually use it (bundled in
  // the build, or installed): ARM64 builds ship without the sidecar.
  const tectonicAvailable = probe.tectonicBundled || Boolean(installed("tectonic"));
  if (latexmk) {
    return {
      text: tectonicAvailable
        ? "latexmk found but no TeX engine. Install a TeX distribution or switch to Tectonic"
        : "latexmk found but no TeX engine. Install a TeX distribution",
      tone: "err",
    };
  }
  return {
    text: tectonicAvailable
      ...
      : "No TeX installation detected. Install a TeX distribution to compile LaTeX",
    tone: "err",
  };
};
  ...
  return (
    <div class="space-y-3">
      ...
      <Card title="Editing" subtitle="Behaviour of the source pane.">
        ...
        <Row
          label="Keybindings"
          hint="Vim or Emacs editing bindings for the source pane. None keeps the standard bindings."
        >
          <SelectStub
            value={

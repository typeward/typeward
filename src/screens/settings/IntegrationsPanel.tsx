...
import {
  ...
  probeZoteroLocalApi,
  probeZoteroServer,
} from "~/integrations/references/zotero";
import { refreshAvailability } from "~/integrations/references/availability";
...
const ZOTERO_LOCAL_PROVIDER_ID = "zotero-better-bibtex";

const BetterBibTexRow: Component = () => {
  // "Ready" if either local path answers — Zotero's HTTP server (which Better
  // BibTeX rides on) or plain Zotero 7's built-in API. All three probes run
  // together: the first two decide reachability, `bbt` only picks the hint.
  const [probe, { refetch }] = createResource(async () => {
    const [server, localApi, bbt] = await Promise.all([
      probeZoteroServer(),
      probeZoteroLocalApi(),
      probeBetterBibTexPlugin(),
    ]);
    return { reachable: server || localApi, bbt };
  });

  // Both caches outlive this row: the provider holds a 60s library/entry TTL
  // and `readyProviders` is probed once per session. Without clearing them,
  // starting Zotero and re-checking here flipped this badge to Ready while the
  // References panel and the sidebar's Refs tab stayed empty.
  const recheck = async () => {
    await refetch();
    citationProviders()
      .find((p) => p.id === ZOTERO_LOCAL_PROVIDER_ID)
      ?.invalidate?.();
    refreshAvailability();
  };
  ...
  return (
    <ProviderRow
      name="Zotero (local)"
      hint="Talks to the Zotero app on this machine; no login needed. Works with plain Zotero 7 (enable 'Allow other applications…' under Settings → Advanced); the Better BibTeX plugin is optional and adds nicer citation keys. Your libraries (personal + groups) are discovered automatically."
      status={
        probe() === undefined
          ? "checking"
          : probe()!.reachable
      ...
      controls={
        <div class="flex items-center gap-1.5">
          <Show when={!probe()?.reachable}>
            ...
            >
              {probe.loading ? "Checking…" : "Re-check"}
            </Button>
          </Show>
          <Switch
